...
function reportView(report: ReviewReleaseControlReport | ReviewPackagePrecheckReport | null, title = '预检报告') {
  ...
  return <div className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-xs text-gray-700">
    <div className="font-semibold">{title}：{report.decision ?? '未知'}</div>
    {findings.length
...
function loadStageLabel(progress: ReviewWorkspaceLoadProgress) {
  const bytes = progress.totalBytes && progress.completedBytes !== undefined ? ` ${Math.min(100, Math.round((progress.completedBytes / progress.totalBytes) * 100))}%` : '';
  return `${progress.message}${bytes}`;
}
 ...
 * behind application-supplied ports.
 */
export function ReviewStatusBoardPanel({ auth, submissionAdapter, releaseControl, onLoadRevision, onClose, subscribeToStatusDraft, subscribeToSubmissionUpload }: ReviewStatusBoardPanelProps) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (session.status !== 'authenticated' || !session.principalId) throw new ReviewOperationError({ code: 'authentication-required', message: '请先在设置中的“登录状态”完成登录。', retryable: false });
      ...
      setMessage(describeOperationError(error));
    } finally {
      setBusy(null);
    }
  ...
  const runPackagePrecheck = useCallback(async () => {
    ...
    try {
      if (!submissionAdapter.precheckSubmission) throw new ReviewOperationError({ code: 'review-package-precheck-not-configured', message: '当前应用没有提供审核包预检适配器。', retryable: false });
      const result = await submissionAdapter.precheckSubmission({
        ...
        targetRevisionId: selectedRevision.revisionId,
        expectedStateVersion: selectedSubmission.stateVersion,
        action: 'precheck',
  ...
  const loadIntoWorkspace = useCallback(async () => {
    ...
    try {
      await onLoadRevision({ submission: selectedSubmission, revision: selectedRevision }, setLoadProgress);
      setLoadProgress({ stage: 'ready', message: '审核工作区已打开。' });
    ...
    try {
      ...
      if (remoteBoard.boardVersion !== board.boardVersion) {
        ...
        if (changed.some((entry) => entry.kind === 'revision-changed')) {
          setMessage('发布前检查已阻断：云端审核包版本已经变化。请刷新对应审核包、检查新版本后保存状态。');
          return;
        }
      ...
      const result = await releaseControl.runReleasePrecheck({
        selectedSubmissionIds: selectedEntries.map((entry) => entry.submissionId),
  ...
  const publish = useCallback(async () => {
    if (!releaseReport?.gate?.attemptId || releaseReport.gate.gateVersion === undefined || !releaseReport.report?.reportSha256 || !publishReady) return;
    ...
    try {
      const result = await releaseControl.confirmRelease({
        attemptId: releaseReport.gate.attemptId,
        expectedGateVersion: releaseReport.gate.gateVersion,
  ...
  }, [actor, publishReady, reload, releaseControl, releaseReport, selectedEntries, submissions]);

  const mutateDetail = useCallback((state: ReviewStatusBoardEntry['state'], reasonPrompt?: string, decisionAction?: ReviewStatusDecisionAction) => {
    if (!selectedSubmission) return;
    const current = draftEntries.find((entry) => entry.submissionId === selectedSubmission.submissionId);
    const reason = reasonPrompt ? window.prompt(reasonPrompt, current?.reason ?? '') ?? undefined : undefined;
    if (reasonPrompt && !reason) return;
    setDraftState(selectedSubmission.submissionId, state, reason, decisionAction);
    setSelectedIds((previous) => new Set(previous).add(selectedSubmission.submissionId));
  }, [draftEntries, selectedSubmission, setDraftState]);
  ...
  return <>
      ...
      <div className="w-[430px] max-h-[76vh] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl" data-draggable-proxy-close="true">
        ...
        <div className="max-h-[calc(76vh-56px)] space-y-3 overflow-y-auto p-3">
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="rounded-xl bg-gray-50 px-3 py-2 text-xs text-gray-600">Gate：{releaseGate ? releaseGate.initialized === false ? '尚未建立' : releaseGate.state : '未读取'} · 已选 {selectedIds.size}</div>
          </div>
            ...
            {submissions.map((submission) => {
              ...
              return <div key={submission.submissionId} className={`rounded-2xl border p-3 ${detailId === submission.submissionId ? 'border-blue-300 bg-blue-50/50' : 'border-gray-200 bg-white'}`}>
                <div className="flex items-start gap-2">
                  <input aria-label={`选择 ${submission.packageName}`} type="checkbox" checked={checked} onChange={() => setSelectedIds((previous) => { const next = new Set(previous); if (next.has(submission.submissionId)) next.delete(submission.submissionId); else next.add(submission.submissionId); return next; })} />
                  <button type="button" onClick={() => { setDetailId(submission.submissionId); setDetailRevisionId(submission.currentRevisionId); setPackageReport(null); }} className="min-w-0 flex-1 text-left"><div className="truncate text-sm font-semibold text-gray-900">{submission.packageName || submission.submissionId}</div><div className="mt-0.5 truncate text-[11px] text-gray-500">{submission.submissionId}</div></button>
          ...
          <div className="grid grid-cols-3 gap-2 border-t border-gray-100 pt-3">
            <AppButton disabled={!selectedEntries.length || busy !== null} onClick={() => void saveStatus()} className="justify-center rounded-xl bg-orange-600 px-2 py-2 text-xs text-white hover:bg-orange-700 disabled:bg-orange-300"><CheckCircle2 className="h-3.5 w-3.5" />保存状态</AppButton>
            <AppButton disabled={!selectedEntries.length || boardDirty || busy !== null} onClick={() => void runReleasePrecheck()} className="justify-center rounded-xl bg-blue-600 px-2 py-2 text-xs text-white hover:bg-blue-700 disabled:bg-blue-300"><ClipboardCheck className="h-3.5 w-3.5" />发布前检查</AppButton>
            <AppButton disabled={!publishReady || busy !== null} onClick={() => void publish()} className="justify-center rounded-xl bg-green-600 px-2 py-2 text-xs text-white hover:bg-green-700 disabled:bg-green-300"><Send className="h-3.5 w-3.5" />发布</AppButton>
          </div>
          {boardDirty ? <div className="text-[11px] text-amber-700">状态灯存在未保存修改；请先保存状态，才可进行发布前检查。</div> : null}
          {reportView(releaseReport, '发布预检报告')}
        </div>
      </div>
    </DraggablePanel>
    {selectedSubmission ? <DraggablePanel id="review-package-detail-panel" defaultPosition={{ x: 468, y: 132 }} zIndex={1765} constrainExpandedToViewport>
      <div className="w-[390px] max-h-[76vh] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl" data-draggable-proxy-close="true">
        ...
        <div className="max-h-[calc(76vh-56px)] space-y-3 overflow-y-auto p-3">
          <div className="flex gap-2"><select className="min-w-0 flex-1 rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800" value={selectedRevision?.revisionId ?? ''} onChange={(event) => { setDetailRevisionId(event.target.value); setPackageReport(null); }}>{selectedSubmission.revisions.map((revision) => <option key={revision.revisionId} value={revision.revisionId}>{revision.revisionId}</option>)}</select><AppButton onClick={() => void reload()} disabled={busy !== null} className="rounded-xl bg-gray-100 px-3 text-xs text-gray-700 hover:bg-gray-200"><RefreshCw className="h-4 w-4" />刷新</AppButton></div>
          ...
          <AppButton onClick={() => void loadIntoWorkspace()} disabled={!selectedRevision || busy !== null} className="w-full justify-center rounded-xl bg-orange-600 px-3 py-2 text-sm font-semibold text-white hover:bg-orange-700 disabled:bg-orange-300"><FileDown className="h-4 w-4" />加载到审核工作区</AppButton>
          <AppButton onClick={() => void runPackagePrecheck()} disabled={!selectedRevision || busy !== null} className="w-full justify-center rounded-xl bg-blue-600 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:bg-blue-300"><ClipboardCheck className="h-4 w-4" />预检</AppButton>
          {reportView(packageReport)}
          <div className="grid grid-cols-3 gap-2 border-t border-gray-100 pt-3"><AppButton onClick={() => mutateDetail('archived', undefined, 'archive')} disabled={busy !== null} className="justify-center rounded-xl bg-gray-100 px-2 py-2 text-xs text-gray-700 hover:bg-gray-200"><Archive className="h-3.5 w-3.5" />归档</AppButton><AppButton onClick={() => mutateDetail('rejected', '请填写要求修改的原因：', 'request-changes')} disabled={busy !== null} className="justify-center rounded-xl bg-red-50 px-2 py-2 text-xs text-red-700 hover:bg-red-100"><XCircle className="h-3.5 w-3.5" />要求修改</AppButton><AppButton onClick={() => mutateDetail('pending', undefined, 'reopen')} disabled={busy !== null} className="justify-center rounded-xl bg-amber-50 px-2 py-2 text-xs text-amber-800 hover:bg-amber-100"><RotateCcw className="h-3.5 w-3.5" />恢复待审</AppButton></div>
          <div className="grid grid-cols-2 gap-2"><AppButton onClick={() => mutateDetail('approved', undefined, 'approve')} disabled={busy !== null} className="justify-center rounded-xl bg-green-600 px-2 py-2 text-xs text-white hover:bg-green-700"><CheckCircle2 className="h-3.5 w-3.5" />通过</AppButton><AppButton onClick={() => mutateDetail('rejected', '请填写打回原因：', 'reject')} disabled={busy !== null} className="justify-center rounded-xl bg-rose-600 px-2 py-2 text-xs text-white hover:bg-rose-700"><XCircle className="h-3.5 w-3.5" />打回</AppButton></div>
        </div>
      </div>
    </DraggablePanel> : null}
    ...
    {loadProgress ? <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40" role="dialog" aria-live="polite"><div className="w-[420px] max-w-[90vw] rounded-2xl border border-gray-200 bg-white shadow-xl"><div className="border-b border-gray-200 px-4 py-3 text-sm font-bold text-gray-900">加载审核工作区</div><div className="px-4 py-4 text-sm text-gray-700">{loadStageLabel(loadProgress)}</div><div className="px-4 pb-4"><div className="h-2 overflow-hidden rounded bg-gray-100"><div className="h-full w-3/5 animate-pulse rounded bg-blue-600" /></div></div></div></div> : null}
  </>;
}
